<%-- 
    Document   : RecordFakultas
    Created on : Sep 16, 2019, 8:35:40 PM
    Author     : ASUS
--%>
...
<%! String idFakultas;%>
<%! String idPT;%>
<% idFakultas = request.getParameter("id-fakultas"); %>
...
<html>
        ...
        <div id="container">
            ...
            <div id="main">
                ...
                <div id="panel-content">
                    ...
                    <!--PANEL EDIT DAN DELETE JIKA FAKULTAS ADMIN-->
                    <div id="submenu-fakultas">
                        
                    </div>
                    
                    <!--DESKRIPSI FAKULTAS-->
                    <form id="form-fakultas" method="POST" action="../UpdateFakultas?id-pt=<% out.print(idPT); %>&id-fakultas=<% out.print(idFakultas); %>" onsubmit="return validasi_input(this)">
                        <div class="div-form">
                             <label class="label-form">Perguruan Tinggi</label>
                             <input id="nama-pt" name="nama-pt" class="unedit-form" type="text"><br/>
                         </div>
                        <div class="div-form">
                             <label class="label-form">Nama</label>
                             <input id="nama-fakultas" name="nama-fakultas" class="input-form" type="text"><br/>
                         </div>
                    ...
                    <!--BUTTON OK-->
                    <div class="div-ok">
                        <input id="button-ok" type="submit" value="OK" class="w3-button w3-round w3-blue button-form" onclick="window.location = 'RecordPT.jsp?id-pt=<% out.print(idPT); %>'">
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            load_fakultas('<% out.print(idFakultas); %>');
        });
        
        /** Memuat Daftar Kelompok Mata Kuliah*/
        function load_fakultas(id = ''){
            $.ajax({
                url:"../GetFakultas",
                method:"POST",
                data:{id: id},
                ...
                success:function(data){
                    if(data.adminFakultas == "YES"){
                        $('#submenu-fakultas').html("<label id=\"label-edit\">Edit</label>"+
                                    "<label class=\"switch\">"+
                                    "<input id=\"check-edit\" type=\"checkbox\">"+
                                    "<span class=\"slider round\"></span>"+
                                    "</label>"+
                                    "<input id=\"button-save\" type=\"submit\" value=\"SAVE\" form=\"form-fakultas\" class=\"w3-button w3-round w3-blue button-pengeditan\">"+
                    ...
                    $('#nama-fakultas').val(data.namaFakultas);
        ...
        function validasi_input(form){
            if($('#nama-fakultas').val() == ""){
                alert("Nama fakultas masih kosong!");
                $('#nama-pt').focus();
                return false;
            } else if($('#alamat').val() == ""){
            ...
            var konfirmasi = window.confirm("Apakah data fakultas sudah benar?\n\n\n"+
                    "Perguruan Tinggi   : "+$('#nama-pt').val()+"\n"+
                    "Fakultas                   : "+$('#nama-fakultas').val()+"\n"+
                    "Alamat                      : "+$('#alamat').val()+"\n"+
