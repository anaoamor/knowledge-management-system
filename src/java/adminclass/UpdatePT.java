...
public class UpdatePT extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            String sql = "UPDATE tbl_pt SET col_nama_pt = ?, col_alamat_pt = ?, col_no_telp_pt = ?, "+
                    "col_email_pt = ? WHERE col_id_pt = ?";
            prepare = koneksi.prepareStatement(sql);
            prepare.setString(1, namaPT);
            prepare.setString(2, alamat);
            prepare.setString(3, noTelepon);
            prepare.setString(4, email);
            prepare.setString(5, idPT);
            int i = prepare.executeUpdate();
            if(i > 0 ){
                out.println("<script> window.location = \'Admin/RecordPT.jsp?id-pt="+idPT+"\';</script>");
            }
