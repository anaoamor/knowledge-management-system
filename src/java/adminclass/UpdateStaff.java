...
public class UpdateStaff extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            //UPDATE RECORD KAPRODI SI
            if(tipeUser.equals("2")){
                sql = "UPDATE tbl_kaprodi_si SET col_nip = ?, "+
                    "col_nama_kaprodi = ?, col_tgl_lahir = ?, col_jenis_kelamin = ?, "+
                    "col_no_hp = ?, col_alamat = ?, col_status = ?, col_jabatan = ?, "+
                    "col_golongan = ?, col_email = ? WHERE col_nip = ?";
                nextPage = "KaprodiSI.jsp";
            }else{
                sql = "UPDATE tbl_wadek_kemahasiswaan SET col_nip = ?, "+
            ...
            out.println("<script>window.location = \'Admin/"+nextPage+"?nip="+nip+"&tipe-user="+tipeUser+"\';</script>");
