 ...
 * @author ASUS
 */
@WebServlet(name = "UpdateUsernamePasswordAdmin", urlPatterns = {"/UpdateUsernamePasswordAdmin"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(rs1.next()){
                ...
                switch(idLevelUser){
                    case 3:{
                        String sql2 = "UPDATE tbl_admin INNER JOIN tbl_user ON "+
                                "(tbl_user.col_id_user = tbl_admin.col_nip) SET tbl_user.col_username = ?, "+
                                "tbl_user.col_password = md5(?), tbl_admin.col_username = ?, "+
                                "tbl_admin.col_password = md5(?) WHERE tbl_user.col_id_user = ?";
                        ...
                        out.println("<script>alert(\'User account berhasil diupdate!\'); window.location = \'Admin/Setelan.jsp\';</script>");
//                      
                    }
                }
            }else{
                out.println("<script>alert(\'Password lama kamu salah!\'); window.location = \'Admin/ChangeUserAccount.jsp\'; </script>");
            }
