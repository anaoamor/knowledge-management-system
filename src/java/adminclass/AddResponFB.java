 ...
 * @author ASUS
 */
@WebServlet(name = "AddResponFBAdmin", urlPatterns = {"/AddResponFBAdmin"})
public class AddResponFB extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            //UPDATE tbl_fb.col_status JIKA STATUS MASIH MENUNGGU
            String sql2 = "Update tbl_fb SET col_status = \'Progress\' WHERE col_id_fb = ? AND col_status = \'Menunggu\'";
            prepare = koneksi.prepareStatement(sql2);
            prepare.setString(1, idFB);
            int j = prepare.executeUpdate();
            
            if(j > 0){
                out.println("<script>window.location = \'Admin/RecordFB.jsp?id-fb="+idFB+"\';</script>");
            }
