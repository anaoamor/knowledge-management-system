 ...
 * @author ASUS
 */
public class UpdateKurikulum extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String jumlahSemester = request.getParameter("jmlh-semester");
        
        //UPDATE RECORD KURIKULUM
        try{
            Connection koneksi = Koneksi.getKoneksi();
            PreparedStatement prepare;
            String sql = "UPDATE tbl_kurikulum SET col_tahun = ?, col_jmlh_sks = ?, "+
                    "col_jmlh_sem = ? WHERE col_id_kurikulum = ?";
            prepare = koneksi.prepareStatement(sql);
            prepare.setString(1, tahun);
            prepare.setString(2, jumlahSKS);
            prepare.setString(3, jumlahSemester);
            prepare.setString(4, idKurikulum);
