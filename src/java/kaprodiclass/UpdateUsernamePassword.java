    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(rs1.next()){
                ...
                switch(idLevelUser){
                    case 2:{
                        String sql2 = "UPDATE tbl_kaprodi_si INNER JOIN tbl_user ON "+
                                "(tbl_user.col_id_user = tbl_kaprodi_si.col_nip) SET tbl_user.col_username = ?, "+
                                "tbl_user.col_password = md5(?), tbl_kaprodi_si.col_username = ?, "+
                                "tbl_kaprodi_si.col_password = md5(?) WHERE tbl_user.col_id_user = ?";
                        ...
                        out.println("<script>alert(\'User account berhasil diupdate!\'); window.location = \'KaprodiSI/Setelan.jsp\';</script>");
//                      
                    }
                }
            }else{
                out.println("<script>alert(\'Password lama anda salah!\'); window.location = \'KaprodiSI/ChangeUserAccount.jsp\'; </script>");
            }
