 ...
 * @author ASUS
 */
public class GetKomentarFD extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //MENDAPATKAN SEMUA KOMENTAR DARI FORUM DISKUSI
        try{
            Connection koneksi = Koneksi.getKoneksi();
            PreparedStatement prepare;
            String sql1 = "SELECT col_detail_komentar, col_tgl, col_waktu, col_id_komentator FROM tbl_komentar "+
                    "WHERE col_id_fd = ?";
            ...
            while(rs1.next()){
                ...
                while(rs2.next()){
                    String idLevelKomentator = rs2.getString("col_id_level");
                    //MENDAPATKAN NAMA KOMENTATOR
                    switch(idLevelKomentator){
                        case "0":{
                            ...
                            while(rs3.next()){
                                m.put("idKomentator", rs1.getString("col_id_komentator"));
                                m.put("namaKomentator", rs3.getString("col_nama_mhs"));
                            }
                            break;
                        } case "2":{
                            String sql3 = "SELECT col_nama_kaprodi FROM "+
                                "tbl_kaprodi_si WHERE col_nip = ?";
                            prepare = koneksi.prepareStatement(sql3);
                            prepare.setString(1, rs1.getString("col_id_komentator"));
                            ...
                            while(rs3.next()){
                                m.put("idKomentator", rs1.getString("col_id_komentator"));
                                m.put("namaKomentator", rs3.getString("col_nama_kaprodi"));
                            }
                            break;
                        }case "3":{
                            ...
                            while(rs3.next()){
                                m.put("idKomentator", rs1.getString("col_id_komentator"));
                                m.put("namaKomentator", rs3.getString("col_nama_admin"));
                            }
                            break;
                        } default:{
                            break;
