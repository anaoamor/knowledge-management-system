...
public class UpdateUsernamePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //AUTENTIFIKASI PASSWAORD LAMA USER
        try{
            ...
            if(rs1.next()){
                String idLevelUser = rs1.getString("col_id_level");
                switch(idLevelUser){
                    case "0":{
                        String sql2 = "UPDATE tbl_mahasiswa INNER JOIN tbl_user ON "+
                                "(tbl_user.col_id_user = tbl_mahasiswa.col_nim) SET tbl_user.col_username = ?, "+
                                "tbl_user.col_password = md5(?), tbl_mahasiswa.col_username = ?, "+
                                "tbl_mahasiswa.col_password = md5(?) WHERE tbl_user.col_id_user = ?";
                        ...
                        out.println("<script>alert(\'User account berhasil diupdate!\'); window.location = \'Setelan.jsp\';</script>");
//                      
                    }
                }
            }else{
                out.println("<script>alert(\'Password lama kamu salah!\'); window.location = \'ChangeUserAccount.jsp\'; </script>");
            }
